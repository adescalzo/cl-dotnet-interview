using FluentValidation.TestHelper;
using TodoApi.Application.Commands.UpdateTodoItem;

namespace TodoApi.Tests.Application.Commands.UpdateTodoItem;

public class UpdateTodoItemCommandValidatorTests
{
    private readonly UpdateTodoItemCommandValidator _validator = new();

    [Fact]
    public void Validator_WhenTodoListIdIsEmpty_ShouldHaveErrorWithEmptyCode()
    {
        // Arrange
        var command = new UpdateTodoItemCommand(Guid.Empty, 1, "Milk");

        // Act
        var result = _validator.TestValidate(command);

        // Assert
        result
            .ShouldHaveValidationErrorFor(x => x.TodoListId)
            .WithErrorCode("todoitems.listid.empty");
    }

    [Theory]
    [InlineData(0)]
    [InlineData(-1)]
    public void Validator_WhenItemIdIsNotPositive_ShouldHaveErrorWithInvalidCode(long itemId)
    {
        // Arrange
        var command = new UpdateTodoItemCommand(Guid.NewGuid(), itemId, "Milk");

        // Act
        var result = _validator.TestValidate(command);

        // Assert
        result.ShouldHaveValidationErrorFor(x => x.ItemId).WithErrorCode("todoitems.id.invalid");
    }

    [Theory]
    [InlineData("")]
    [InlineData(" ")]
    public void Validator_WhenNameIsBlank_ShouldHaveErrorWithEmptyCode(string name)
    {
        // Arrange
        var command = new UpdateTodoItemCommand(Guid.NewGuid(), 1, name);

        // Act
        var result = _validator.TestValidate(command);

        // Assert
        result.ShouldHaveValidationErrorFor(x => x.Name).WithErrorCode("todoitems.name.empty");
    }

    [Fact]
    public void Validator_WhenNameExceedsMaximumLength_ShouldHaveErrorWithTooLongCode()
    {
        // Arrange
        var command = new UpdateTodoItemCommand(Guid.NewGuid(), 1, new string('a', 201));

        // Act
        var result = _validator.TestValidate(command);

        // Assert
        result.ShouldHaveValidationErrorFor(x => x.Name).WithErrorCode("todoitems.name.too_long");
    }

    [Fact]
    public void Validator_WhenCommandIsValid_ShouldNotHaveAnyErrors()
    {
        // Arrange
        var command = new UpdateTodoItemCommand(Guid.NewGuid(), 1, "Milk");

        // Act
        var result = _validator.TestValidate(command);

        // Assert
        result.ShouldNotHaveValidationErrorFor(x => x.TodoListId);
        result.ShouldNotHaveValidationErrorFor(x => x.ItemId);
        result.ShouldNotHaveValidationErrorFor(x => x.Name);
    }
}
